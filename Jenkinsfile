...
pipeline{
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.128.206.55 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://18.218.128.243:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#aaj-devsecops-cicd-alert', //update and provide your channel name
